...
class AccCubicRegNewton(Optimizer):
    ...
    def __init__(self, fun, x0, gradtol, maxite, maxorcs, alpha0, M0):
        self.info = STATS
        self.M = M0
        self.x0, self.xk = x0, x0
        super().__init__(fun, x0, alpha0, gradtol, maxite, maxorcs)
        
    def step(self):
        if self.k == 0:
            self.xk, self.cubicIte, self.cubicOpt, self.total_cubic_oracle = self.GDSolvesCubic(self.x0, self.M)
            self.fk, self.gk = self.fun(self.xk, "01")
            self.fs = self.fk.reshape(1)
            self.auxIte = 0
            self.auxOpt = 0
            ...
            self.xk, self.cubicIte, self.cubicOpt, self.total_cubic_oracle = self.GDSolvesCubic(self.yk, 2 * self.M)
            ...
            self.vk, self.auxIte, self.auxOpt = self.GDSolvesAuxF(6 * self.M)
    ...
    def GDSolvesAuxF(self, M, eps = 1e-9, TMax = 10000):
        # initialization 
        vk = self.vk
